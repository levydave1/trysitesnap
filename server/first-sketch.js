...
function rawFacts(fields) {
  return {
    ...
    mainPhoto: text(fields["Main Photo URL"] || fields["Main Photo"]),
    facebook: text(fields["Company Facebook"]),
    instagram: text(fields["Company Instagram"]),
    linkedin: text(fields["Company LinkedIn"]),
    youtube: text(fields["Company YouTube"]),
    bookingAppointmentLink: text(fields["Booking Appointment Link"]),
    reservationLinks: text(fields["Reservation Links"]),
    menuLink: text(fields["Menu Link"]),
    orderLinks: text(fields["Order Links"]),
    about: text(fields.About || fields["About JSON"] || fields.Description || fields["Website Description"]),
...
Use only the following controlled design vocabulary so the brief gives the HTML model the same concrete range of choices as the original Make scenario:
- DESIGN_ARCHETYPE: BOLD_INDUSTRIAL, TRUSTED_PROFESSIONAL, CLEAN_MODERN, EARTHY_ORGANIC, LUXURY_DARK, VIBRANT_LOCAL, WARM_FAMILY, MINIMAL_PREMIUM, PLAYFUL_CREATIVE or TECH_SHARP.
- PALETTE_ID: navy_ice_white, emerald_mint_stone, burgundy_blush_cream, sage_sand_charcoal, terracotta_cream_espresso, cobalt_white_gray, charcoal_lime_concrete, lavender_slate_white, forest_beige_copper, red_cream_navy, teal_pearl_graphite, plum_champagne_ivory, sky_blue_cloud_white, olive_clay_linen, monochrome_red_accent, slate_cobalt_white, ivory_charcoal_sage, concrete_blue_lime, cream_forest_clay or graphite_teal_white.
- LAYOUT_VARIANT: centered_hero, split_hero_image_left, split_hero_image_right, card_stack_hero, editorial_hero, diagonal_hero, image_overlay_hero, minimal_business_card, bold_local_landing or soft_service_landing.
- HERO_STYLE: large_photo_background, clean_white_with_card, dark_panel_with_accent, editorial_text_first, local_trust_badge_focus, service_problem_solution, premium_minimal or colorful_block_intro.
- SECTION_STYLE: rounded_cards, sharp_cards, soft_gradient_sections, bordered_editorial_sections, alternating_image_text, compact_mobile_cards, masonry_gallery_feel or trust_bar_sections.
- IMAGE_TREATMENT: full_bleed_photos, rounded_corner_photos, black_and_white_with_color_accent, warm_overlay, cool_overlay, clean_no_overlay, polaroid_cards or editorial_crop.
- NEUTRAL_FAMILY: slate, zinc, neutral, stone or gray. BACKGROUND_MODE: light, dark, mixed, warm or colorful. BRAND_COLOR_STRATEGY: preserve, reinterpret or replace.
- FONT_PAIR: Poppins + Inter, Montserrat + Inter, Playfair Display + Inter, Lora + Source Sans 3, Manrope + Inter, DM Sans + Lora, Oswald + Inter, Merriweather + Inter or Nunito Sans + Inter.

Select PALETTE_ID first, then make the actual colors match its name. Use preserve only when the existing colors are already clear and professional, reinterpret when they are useful but need refinement, and replace when they are weak or irrelevant. LUXURY_DARK is reserved for businesses that clearly signal luxury or exclusivity. Explain the category fit and the chosen composition in CLAUDE_DESIGN_INSTRUCTIONS so the next model does not fall back to the same split hero and repeated card grid.
...
- Mobile Hero must contain a visible foreground photo card inside the Hero, aspect 4/3, never hidden at mobile. A background photo alone does not count. Use object-cover, never reduce the main image below 60% opacity, and use a readable text panel/overlay; no excessive viewport-height whitespace. If the desktop Hero occupies most of the first viewport, include a subtle scroll cue to #services.
...
- Use exact rating/review count only when supplied. Never invent or round; avoid emphasizing a review count below 50 unless the source makes it important. Use real testimonials only; never invent quotes, reviewers or initials. If none exist, use factual trust/value cards without quotation marks. Mobile testimonials use native horizontal swipe (overflow-x-auto, snap-x, cards min-width about 85vw), visible accessible left/right controls and lightweight JavaScript; controls are hidden or unnecessary on the desktop grid.
- About is concise and editorial, not another centered card grid. Use a safe real photo with a 4/3 crop when one is available.
...
SERVICES MOBILE QA IS MANDATORY: preserve 4-or-6 service composition with featured photo cards. For every compact service card, row 1 must be icon + title on the same horizontal row and row 2 the description below the whole row. Compact cards must render two per row on mobile, with short text-sm descriptions and no tall empty spacing. Process has exactly three steps and uses the same number/icon + title row rule on mobile while remaining visually distinct from Services. Testimonials use native horizontal swipe on mobile and desktop grid when real testimonials exist; add visible accessible left/right mobile controls with lightweight JavaScript and hide them on desktop.
...
Final verification: count the actual main service cards after repair and keep exactly 4 or exactly 6 (never 5, 7 or a CTA disguised as a service); all required sections survive; compact service and process mobile rows are correct; no form or old-site links remain; map/logo/images/phones are valid; FAQ JSON-LD and category note appear once; motion is fail-safe; HTML is complete.`;
}
...
${anchorNavigation}`;
  ...
  const experience = `<script data-sitesnap-experience>(function(){
var o=document.getElementById("sitesnap-design-notes"),b=document.querySelector(".sitesnap-preview-cta"),c=document.querySelector(".sitesnap-design-notes-close");
function set(v){if(!o||!b||!c)return;o.dataset.open=v?"true":"false";if(v)c.focus();else b.focus()}
if(o&&b&&c){b.addEventListener("click",function(){set(true)});c.addEventListener("click",function(){set(false)});o.addEventListener("click",function(e){if(e.target===o)set(false)});document.addEventListener("keydown",function(e){if(e.key==="Escape"&&o.dataset.open==="true")set(false)})}
var sections=[].slice.call(document.querySelectorAll("section")).filter(function(x){return x.id!=="finalize-section"&&x.id!=="sitesnap-location"});
var patterns=["sitesnap-pattern-radial","sitesnap-pattern-dots","sitesnap-pattern-grid"];
sections.forEach(function(x,i){if(getComputedStyle(x).backgroundImage==="none")x.classList.add(patterns[i%patterns.length])});
var services=document.getElementById("services")||sections.find(function(x){var h=x.querySelector("h2");return h&&/services|offerings|what we do|how we help/i.test(h.textContent||"")});
if(services){
  if(!services.id)services.id="services";
  var nodes=[].slice.call(services.querySelectorAll("article,div,li"));
  function titleOf(card){var heading=card.querySelector("h3,h4");if(heading)return heading;return [].slice.call(card.querySelectorAll("span")).find(function(x){var value=(x.textContent||"").trim();return value&&value.length<90&&/(font-heading|font-bold|font-semibold|font-extrabold)/i.test(x.className||"")})||null}
  function iconOf(card){return card.querySelector('i[data-lucide],svg,[class*="icon"]')}
  function compactBase(card){return !card.querySelector("img,picture")&&!!titleOf(card)&&!!iconOf(card)&&(card.textContent||"").trim().length>12}
  var compact=nodes.filter(function(card){if(!compactBase(card)||!card.parentElement)return false;return [].slice.call(card.parentElement.children).filter(compactBase).length>1});
  compact.forEach(function(card){var title=titleOf(card),icon=iconOf(card);card.classList.add("sitesnap-compact-card");if(!title||!icon)return;var common=icon.parentElement;while(common&&common!==card&&!common.contains(title))common=common.parentElement;if(common&&common!==card&&!common.querySelector("p")){common.classList.add("sitesnap-service-head");return}var shell=icon.parentElement&&icon.parentElement!==card&&!icon.parentElement.querySelector("h3,h4,p")?icon.parentElement:icon;var head=document.createElement("div");head.className="sitesnap-service-head";title.parentNode.insertBefore(head,title);head.appendChild(shell);head.appendChild(title)});
  var parents=[];compact.forEach(function(card){if(card.parentElement&&parents.indexOf(card.parentElement)<0)parents.push(card.parentElement)});parents.forEach(function(parent){if(compact.filter(function(card){return card.parentElement===parent}).length>1)parent.classList.add("sitesnap-compact-grid")});
  var imageCards=nodes.filter(function(card){return !!card.querySelector("img,picture")&&!!titleOf(card)&&(card.textContent||"").trim().length>20});
  imageCards=imageCards.filter(function(card){return !imageCards.some(function(other){return other!==card&&card.contains(other)})});
  var serviceCards=compact.concat(imageCards.filter(function(card){return compact.indexOf(card)<0}));
  serviceCards=serviceCards.filter(function(card,index,list){return list.indexOf(card)===index}).sort(function(a,d){return a.compareDocumentPosition(d)&Node.DOCUMENT_POSITION_FOLLOWING?-1:1});
  var target=serviceCards.length===5?4:(serviceCards.length>6?6:serviceCards.length);
  serviceCards.slice(target).forEach(function(card){card.hidden=true;card.setAttribute("data-sitesnap-extra-service","hidden")});
  serviceCards.slice(0,target).forEach(function(card){card.setAttribute("data-sitesnap-service-card","true")});
}
var photos=[].slice.call(document.images).filter(function(img){return !img.closest("header")&&!img.classList.contains("sitesnap-brand-logo")});
function validPhoto(img){return img.complete&&img.naturalWidth>=240&&img.naturalHeight>=160}
function repairPhoto(img,finalAttempt){if(validPhoto(img))return;if(img.dataset.sitesnapImageRepair==="done"){if(finalAttempt)img.style.display="none";return}var replacement=photos.find(function(candidate){return candidate!==img&&validPhoto(candidate)});if(replacement){img.dataset.sitesnapImageRepair="done";img.src=replacement.currentSrc||replacement.src;return}if(finalAttempt){img.dataset.sitesnapImageRepair="done";img.style.display="none";img.setAttribute("aria-hidden","true")}}
photos.forEach(function(img){img.addEventListener("error",function(){setTimeout(function(){repairPhoto(img,false)},250)},{once:true})});
setTimeout(function(){photos.forEach(function(img){if(img.complete&&img.naturalWidth===0)repairPhoto(img,true)})},2500);
sections.slice(1).forEach(function(x){x.classList.add("sitesnap-reveal")});
if("IntersectionObserver" in window){var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){e.target.classList.add("sitesnap-visible");io.unobserve(e.target)}})},{threshold:.08});sections.slice(1).forEach(function(x){io.observe(x)})}else{sections.forEach(function(x){x.classList.add("sitesnap-visible")})}
var hero=sections[0]&&sections[0].querySelector("img");if(hero)hero.classList.add("sitesnap-hero-image")
})();</script>`;
...
#services [data-sitesnap-extra-service="hidden"]{display:none!important}#services .sitesnap-compact-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:clamp(8px,2vw,16px)!important;align-items:stretch!important}#services .sitesnap-compact-card{min-width:0!important;width:auto!important;min-height:0!important;height:auto!important;padding:clamp(12px,2vw,20px)!important}#services .sitesnap-service-head{display:flex!important;align-items:center!important;gap:clamp(8px,1.5vw,12px)!important;min-width:0!important}#services .sitesnap-service-head>h3,#services .sitesnap-service-head>h4,#services .sitesnap-service-head>span{margin:0!important;min-width:0!important;line-height:1.2!important}#services .sitesnap-compact-card>p,#services .sitesnap-compact-card .sitesnap-service-description{margin-top:10px!important}@media(max-width:480px){#services .sitesnap-compact-card{padding:12px!important}#services .sitesnap-service-head{gap:8px!important}#services .sitesnap-service-head>h3,#services .sitesnap-service-head>h4,#services .sitesnap-service-head>span{font-size:clamp(14px,4.1vw,17px)!important}#services .sitesnap-compact-card p{font-size:14px!important;line-height:1.45!important}}
