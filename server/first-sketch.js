...
#services .sitesnap-compact-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:clamp(8px,2vw,16px)!important;align-items:stretch!important}#services .sitesnap-compact-card{min-width:0!important;width:auto!important;min-height:0!important;height:auto!important;padding:clamp(12px,2vw,20px)!important}#services .sitesnap-service-head{display:flex!important;align-items:center!important;gap:clamp(8px,1.5vw,12px)!important;min-width:0!important}#services .sitesnap-service-head>h3,#services .sitesnap-service-head>h4{margin:0!important;min-width:0!important;line-height:1.2!important}#services .sitesnap-compact-card>p,#services .sitesnap-compact-card .sitesnap-service-description{margin-top:10px!important}@media(max-width:480px){#services .sitesnap-compact-card{padding:12px!important}#services .sitesnap-service-head{gap:8px!important}#services .sitesnap-service-head>h3,#services .sitesnap-service-head>h4{font-size:clamp(14px,4.1vw,17px)!important}#services .sitesnap-compact-card p{font-size:14px!important;line-height:1.45!important}}
...
<script data-sitesnap-experience>(function(){var o=document.getElementById("sitesnap-design-notes"),b=document.querySelector(".sitesnap-preview-cta"),c=document.querySelector(".sitesnap-design-notes-close");function set(v){o.dataset.open=v?"true":"false";if(v)c.focus();else b.focus()}b.addEventListener("click",function(){set(true)});c.addEventListener("click",function(){set(false)});o.addEventListener("click",function(e){if(e.target===o)set(false)});document.addEventListener("keydown",function(e){if(e.key==="Escape"&&o.dataset.open==="true")set(false)});var sections=[].slice.call(document.querySelectorAll("section")).filter(function(x){return x.id!=="finalize-section"&&x.id!=="sitesnap-location"});var patterns=["sitesnap-pattern-radial","sitesnap-pattern-dots","sitesnap-pattern-grid"];sections.forEach(function(x,i){if(getComputedStyle(x).backgroundImage==="none")x.classList.add(patterns[i%patterns.length])});var services=document.getElementById("services")||sections.find(function(x){var h=x.querySelector("h2");return h&&/services|offerings|what we do|how we help/i.test(h.textContent||"")});if(services){if(!services.id)services.id="services";var compact=[].slice.call(services.querySelectorAll("article")).filter(function(card){return !card.querySelector("img,picture")&&card.querySelector("h3,h4")});compact.forEach(function(card){var title=card.querySelector("h3,h4"),icon=card.querySelector('i[data-lucide],svg,[class*="icon"]');card.classList.add("sitesnap-compact-card");if(!title||!icon)return;var common=icon.parentElement;while(common&&common!==card&&!common.contains(title))common=common.parentElement;if(common&&common!==card&&!common.querySelector("p")){common.classList.add("sitesnap-service-head");return}var shell=icon.parentElement&&icon.parentElement!==card&&!icon.parentElement.querySelector("h3,h4,p")?icon.parentElement:icon;var head=document.createElement("div");head.className="sitesnap-service-head";title.parentNode.insertBefore(head,title);head.appendChild(shell);head.appendChild(title)});var parents=[];compact.forEach(function(card){if(card.parentElement&&parents.indexOf(card.parentElement)<0)parents.push(card.parentElement)});parents.forEach(function(parent){if(compact.filter(function(card){return card.parentElement===parent}).length>1)parent.classList.add("sitesnap-compact-grid")})}sections.slice(1).forEach(function(x){x.classList.add("sitesnap-reveal")});if("IntersectionObserver" in window){var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){e.target.classList.add("sitesnap-visible");io.unobserve(e.target)}})},{threshold:.08});sections.slice(1).forEach(function(x){io.observe(x)})}else{sections.forEach(function(x){x.classList.add("sitesnap-visible")})}var hero=sections[0]&&sections[0].querySelector("img");if(hero)hero.classList.add("sitesnap-hero-image")})();</script>
${tracker}
${anchorNavigation}`;
